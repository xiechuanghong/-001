...
function doUpload(filePath, success, fail, options) {
    ...
    if (!config.qiniuShouldUseQiniuFileName) {
      formData['key'] = options.prefix+fileName
    }
    wx.uploadFile({
        url: url,
        filePath: filePath,
        ...
        success: function (res) {
          ...
          if ((dataString == 'null' || dataString == null) && res.errMsg =='uploadFile:ok' ){
            return success(res);
          }
          try {
...
function getQiniuToken(callback, key) {
  wx.request({
    ...
    success: function (res) {
      var token = res.data.data.token; console.log(res.data.uptoken); console.log(res.data)
